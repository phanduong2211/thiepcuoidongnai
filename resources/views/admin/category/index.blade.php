...
var iFrame="<?php if(isset($_GET['iframe'])) echo 'true'; else echo 'false'; ?>"
    function callBackSuccessModal(data){
         if(dataitem.action=="addnew"){
            ...
            if(iFrame=="true"){
              $('#addnewct', window.parent.document).parent().find("select").append("<option value='"+data.id+"'>"+data.name+"</option>").val(data.id);
              $('#dialogct .closedialog', window.parent.document).click();
            }
        }else{
...
<div class="row">
    <div class="col-lg-4 col-md-4 col-sm-4 col-xs-12" style="margin-bottom:5px">
       <div class="group-button clearfix">
           <a href="#addnew" dataitem='{"action":"addnew","title":"Thêm Mới","url":"{{Asset('admin/category/add')}}"}' data-toggle="modal" id="addnewitem" data-target="#modaldialog" class="pull-left btn btn-primary btn-sm">Thêm mới</a>
       </div>
   </div>
    ...
    <div class="modal-content">
      ...
      <div class="modal-body">
            ...
            <form method="post" action="{{Asset('admin/category/add')}}">
                <div class="row">
                    <div class="col-xs-4">Tên Loại Sản Phẩm:</div>
